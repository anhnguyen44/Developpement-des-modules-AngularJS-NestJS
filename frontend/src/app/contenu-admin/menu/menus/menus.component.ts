...
import {TypeMenuService} from '../../../resource/menu/type-menu.service';
import {NotificationService} from '../../../notification/notification.service';
import {ITypeMenu, Profil, IProfil, CMenuProfil, IDroit, IMenuDefini} from '@aleaac/shared';
...
import {MenuProfilService} from '../../../resource/profil/menu-profil.service';
import {MenuProfil} from '../../../resource/profil/MenuProfil';
import {DroitService} from '../../../resource/droit/droit.service';
import {MenuDroitService} from '../../../resource/droit/menu-droit.service';
...
export class MenusComponent implements OnInit {
    test1 = 'ICONE';
    listeTypeMenu: ITypeMenu[];
    menuForm = this.formBuider.group({
        id: [null, null],
        menuParent: [''],
    ...
    constructor(
        ...
        private typemenuServie: TypeMenuService,
    ...
    ngOnInit() {
        this.typemenuServie.getAllTypeMenu().subscribe((data1) => {
            this.listeTypeMenu = data1;
        }, err => {
        ...
        this.menuService.setMenu([
            ...
            ['Menus', '/contenu-admin/menu/liste-menu'],
            ['Nouveau Menu', '']
        ]);
        if (!this.id) {
            this.menuCreate = new MenuDefini();
            this.menuService.getAllMenusNonRecher().subscribe((data4) => {
                this.listeMenuNonRecher = data4;
            }, err => {
                this.notificationService.setNotification('danger', ['Une erreur est survenue.']);
            });
        } else {
            this.menuService.getMenuById(this.id).subscribe((data2) => {
                this.menuCreate = data2;
                this.InitForms();
            }, err => {
                this.notificationService.setNotification('danger', ['Une erreur est survenue.']);
                console.log(err);
            ...
            this.menuService.getAllMenusNonRecher().subscribe((data4) => {
                this.listeMenuNonRecher = data4;
                this.listeMenuNonRecher.forEach(mNonRe => {
                    if (mNonRe.id == this.menuCreate!.id) {
                        this.listeMenuNonRecher = this.listeMenuNonRecher!.filter(item => item.id !== mNonRe.id);
                    }
                }, err => {
                    this.notificationService.setNotification('danger', ['Une erreur est survenue.']);
                    console.log(err);
        ...
        this.menuService.getAllMenus(this.queryBuild).subscribe((data3) => {
            this.listeMenu = data3;
        }, err => {
            this.notificationService.setNotification('danger', ['Une erreur est survenue.']);
        });
    ...
    openModalIcone() {
        this.modalIcone = true;
        console.log(this.modalIcone);
    }

    private InitForms() {
        this.menuForm.patchValue(this.menuCreate!);
    }

    validateMenu() {
        this.submitedMenu = true;

        if (this.menuForm.invalid) {
            this.errorsMenu = [];
            this.errorsMenu = this.validationService.getFormValidationErrors(this.menuForm, this.champsMenu);
            this.notificationService.setNotification('danger', this.errorsMenu);
            return false;
        } else {
            return true;
        }
    ...
    onSubmitMenu() {
        ...
        if (this.menuForm.value.menuParent == '') {
            this.menuForm.value.menuParent = null;
        }
        if (this.menuForm.get('droitsForMenu')!.value == 'null') {
            this.menuForm.value.droitsForMenu = null;
        }
        this.menuCreate = {...this.menuCreate, ...this.menuForm.value};

        if (!this.id) {
            this.menuService.createMenu(this.menuForm.value).subscribe((data) => {
                this.notificationService.setNotification('success', ['Menu créé.']);
                this.router.navigate(['/contenu-admin', 'menu', 'liste-menu']);

            }, err => {
                this.notificationService.setNotification('danger', ['Une erreur est survenue.']);
            });
        } else {
            console.log(this.menuCreate);
            this.menuService.updateMenu(this.menuForm.value).subscribe((dataUpdate) => {
                this.notificationService.setNotification('success', ['Menu mis à jour.']);
                this.router.navigate(['/contenu-admin', 'menu', 'liste-menu']);
            }, err => {
                this.notificationService.setNotification('danger', ['Une erreur est survenue.']);
            });
    ...
    setUrlMenuParent() {
        const url = this.menuForm.value.menuParent.url;
        const titreValue = this.escapeRegExp(this.menuForm.value.titre);
        if (titreValue) {
            if (url) {
                this.menuForm.controls['url'].setValue(url + '/' + titreValue);
            } else {
                this.menuForm.controls['url'].setValue('/' + titreValue);
            }
        } else {
            if (url) {
                this.menuForm.controls['url'].setValue(url);
            } else {
    ...
    titreToUrl(s: string) {
        ...
        const urlParent = this.menuForm.value.menuParent.url;
        if (!urlParent) {
            if (s) {
                this.menuForm.controls['url'].setValue('/' + s);
                return '/' + s;
            }
            return;
        } else {
            if (s) {
                this.menuForm.controls['url'].setValue(urlParent + '/' + s);
                return urlParent + '/' + s;
            } else {
                this.menuForm.controls['url'].setValue(urlParent);
                return urlParent;
            }
        }
    }
    ...
    setIcone(classIcone: string) {
        // console.log(classIcone);
        this.menuForm.patchValue({icone: classIcone});
        this.modalIcone = false;
    }
