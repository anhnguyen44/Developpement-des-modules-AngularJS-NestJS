...
export class ResourceService {
  ...
  updateResource(resource: Resource, resourceName: string): Observable<Resource> {
    const $data = this.http
      .patch(this.resourcesUrl(resourceName) + '/' + resource.id, resource)
      .pipe(
        map((resp: any) => resp.data),
        share(), take(1)
      );
  ...
  updatePutResource(resource: Resource, resourceName: string): Observable<Resource> {
    const $data = this.http
      .put(this.resourcesUrl(resourceName) + '/' + resource.id, resource)
      .pipe(
        map((resp: any) => resp.data),
        share()
      );
